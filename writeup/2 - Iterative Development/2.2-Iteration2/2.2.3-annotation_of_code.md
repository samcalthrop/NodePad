...
**_TRIGGER WARNING: at test 3, there are flashing lights due to reloading the page continuously_**

For Iteration 2, I made progress in the following general areas:

- [Bug Fixing](#errors-carried-over-from-iteration-1)
  - [Sidebar - Fixing the inconsistency of the Sidebar](#using-a-provider)
  - [Text Editor - Allowing the Text Editor to render entire files correctly](#mdxeditor-methods-and-refs)
...
<div align="center">
  <img src="./src/reloadNetwork.gif" alt="">
</div>
...
| Description                                                                | Input                                                      | Output                                                                        | Result         |
| -------------------------------------------------------------------------- | ---------------------------------------------------------- | ----------------------------------------------------------------------------- | -------------- |
| Test that a random position is generated every time the function is called | Navigate to the Home screen and reload the window 10 times | See that random positions are generated for each node every time successfully | **Success** ✅ |

**Evidence:**

Since this test is done in conjunction with Test 3, the evidence is also the same (_see Test 3 above_).
...
| Description                                                                                                                 | Input                                                                                                                                                            | Output                                                  | Result         |
| --------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------- | -------------- |
| Test that the Network is re-generated each time the files passed into the Network change e.g. the root directory is changed | Since I have no way of directly changing the source directory, I will change the path to the root directory statically in the code whilst the program is running | Node Network should be updated to display the new files | **Success** ✅ |

**Evidence:**

<div align="center">
  <img src="./src/changePath.gif" alt="">
</div>
...
| Description                                                                                                                                                                                                                                                 | Input                                                                  | Output                                                  | Result         |
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------- | ------------------------------------------------------- | -------------- |
| Ensure that all nodes are drawn as blue circles of radius 15px, with smaller white circles within them, a dotted grey line should be drawn between a mouse and a node if a connection is being drawn from the node, and connections should be drawn in grey | Navigate to the Home screen and draw connections between several nodes | The criteria described in the Description should be met | **Success** ✅ |

_Note: this test assumes the code for drawing connections is fully functional (i.e. the `handleMouseDown()` function works)_

**Evidence:**

<div align="center">
  <img src="./src/drawingWorks.gif">
</div>
...
| Description                                                                                                | Input                                                                                     | Output                                                                       | Result         |
| ---------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------- | -------------- |
| Check that connections are removed if, and only if, the mouse is within the bounds of the connection line. | Draw a connection, then first click outside of the connection, and then on the connection | Connection should only be removed once the connection is directly clicked on | **Success** ✅ |

**Evidence:**

<div align="center">
  <img src="./src/isClickOnConnectionWorks.gif" alt="">
</div>
...
| Description                                                                                                                                                                                                                | Input                                                                                                                   | Output                                                                                            | Result         |
| -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- | -------------- |
| Test that the cases that the mouse is clicked work: clicking and holding on the blue part of a node drags it, clicking and holding on the white part of a node drags a connection, and clicking on a connection deletes it | Try each of the three cases in the description (_try dragging a node, dragging a connection and removing a connection_) | The attempts at each case should be successful _i.e. do what the Description says they should do_ | **Success** ✅ |

**Evidence:**

<div align="center">
  <img src="./src/mouseDownWorks.gif">
</div>
...
| Description                                                                                                                                                                                                        | Input                                               | Output                                                                                                                                        | Result         |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- | -------------- |
| Check that if the left mouse button is being held down, any mouse movements are reflected in the network: if a connection is being dragged, that is seen visually, and if a node is being dragged the same is true | Try dragging a node, and then dragging a connection | The a connection should be visually dragged to follow the mouse (_as a dotted line_), and the node should also be dragged to follow the mouse | **Success** ✅ |

**Evidence:**

The evidence can be found in the evidence for Test 8 (_see above_).
...
| Description                                                                                                                                                                                                                       | Input                                                                                                                                | Output                                              | Result         |
| --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------- | -------------- |
| Check that when the left mouse button is lifted: if a node is being dragged, stop dragging, and if a connection is being dragged it should be cancelled if not above a node, otherwise drawing a connection between the two nodes | Try dragging and dropping a node, then do the same with a connection, first dropped in empty space, and then drawn between two nodes | The cases specified in the Description should occur | **Success** ✅ |

**Evidence:**

<div align="center">
  <img src="./src/mouseUpWorks.gif">
</div>
...
| Description                                                               | Input                                             | Output                                                     | Result         |
| ------------------------------------------------------------------------- | ------------------------------------------------- | ---------------------------------------------------------- | -------------- |
| Test that resizing the window doesn't change the size/ scale of the nodes | Navigate to the Home screen and resize the window | Nodes should stay the same size, and retain their position | **Success** ✅ |

**Evidence:**

<div align="center">
  <img src="./src/resizingWorks.gif">
</div>
...
Given all the methods on the canvas were tested in isolation, and these changes were optimisations, the only testing to be done was see how the Network performed over time.

A comparison of before the optimisations vs. after is below:

#### Before

<img src='./src/beforeOptimisations.gif' />

#### After

<img src='./src/afterOptimisations.gif' />

As we can see, these optimisations have drastically improved both short and long term performance in the network, especially addressing the drop off in frames seen after a few seconds in the before screen recording.

At this point, I found myself out of time for this iteration.
