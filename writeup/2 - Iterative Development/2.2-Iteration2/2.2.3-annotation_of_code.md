...
This function checks whether the user has clicked a connection (this will cause the connection to be removed). It does this by first calculating the perpendicular distance from the click to the connection between the two nodes using vector calculus (_vector cross product_), and ensuring this is a suitably small amount so as to make the user experience smooth. It then calculates the angle between the connection line and the horizontal axis, and checks that the click is within the line segment of the connection, as well as the node's radius - this prevents the program from removing the node is the user clicks on the node itself.
