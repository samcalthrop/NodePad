...
- takes user to signup screen when button pressed
...
The Signup screen is a close copy of the login screen with a few differences; therefore the code (*for the frontend at least*) looks very similar for the most part:
```tsx
...
- takes user to login page when button pressed
        ...
        <div className={classes.thing}>
          ...
          <div className={classes.mdxeditor}>
            <MDXEditor
              ...
              markdown={fileContents ?? '# Hello world'}
...
/>
```
allowing unsafe scripts to be run from specified sources (*in this case, unsafe scripts can be run from the <MDXEditor> source code - https://github.com/mdx-editor/editor*)

#### Testing

#### The result
After successfully debugging the issue, the editor looks like this (I highloghted the background in red to make the area more visible):

<img src="./src/editnodemetasuccess.png"/>
...
- File Contents Retrieval
  ...
  const path = './README.md';
  ```
  ^In next iteration, I will make something to allow this to be sent from frontend, and be custom selected by user
