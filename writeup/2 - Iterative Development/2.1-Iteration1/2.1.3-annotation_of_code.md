...
This cannot be a full success yet as I have not created all screens within the app yet. This will be tackled either later this iteration or in the next one.
...
| Description                                                                                     | Input                    | Output                             | Result         |
| ----------------------------------------------------------------------------------------------- | ------------------------ | ---------------------------------- | -------------- |
| The user should be able to navigate to and from the Signup screen by means of a sign up button. | Click the sign up button | User is taken to the Signup screen | **Success** ✅ |
...
| Description                                                                                                                                                                                                     | Input                                                                                                                                                                                                                                                                                       | Output                                                                                                                                                                                                        | Result                                                                                                                 |
| --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| The user must enter valid signup data to be accepted as a new user (`email` must contain an `@` and a proceeding `.`, the password must be ≥ 8 characters long _and_ the two password fields must be identical) | An email not containing and `@` or a `.`, first without and then with a password < 8 characters long and then one ≥ 8 characters long, then keep the previous credentials in the fields and use first a password that doesn't match the first password, and then a password that does match | Only in the case that the entered details match the format specified in the Description should the used be signed up and navigated back the the Login screen, otherwise an appropriate error should be thrown | **Failure** ❌ - _currently, entering anything into the fields, or even nothing at all results in a successful signup_ |
...
I aimed to have a back button in place next iteration, as it wasn't a priority for me at that point.
...
| Description                                                                                                                                                                                     | Input                                                                      | Output                                                                                                                | Result         |
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- | -------------- |
| When leaves of the tree are clicked on: if the leaf is a folder, the leaf should be expanded and its children shown indented below it, with the closed folder icon becoming an open folder icon | Left click on folders in the Sidebar, first to open and then to close them | Folders should expand and display child contents upon the first click, and retract the children upon the second click | **Success** ✅ |
...
| Description                                                                             | Input                 | Output                                        | Result                                                                                                                                                                                                                        |
| --------------------------------------------------------------------------------------- | --------------------- | --------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| When the filetree is collapsed/ expanded, the Sidebar's size/ styling should not change | Resize the app window | Ensure the criteria in the Description is met | **Failure** ❌ - _the Sidebar becomes more elongated when files are expanded, even going off the page sometimes, and the original size is achieved by collapsing these folders - the Sidebar should remain a consistent size_ |
...
These tests both resulted in failure. As I had very little time to fix these problems given what I had left to do in the iteration, I decided these were problems to be solved in the next iteration.
...
| Description                                                                                                 | Input                            | Output                                                            | Result         |
| ----------------------------------------------------------------------------------------------------------- | -------------------------------- | ----------------------------------------------------------------- | -------------- |
| Check that the file clicked on in the Sidebar has the correct contents retrieved from the user's filesystem | Left click a file in the Sidebar | The console on the backend should have the contents printed to it | **Success** ✅ |
...
| Description                                                                                                           | Input                            | Output                                                                                                                         | Result         |
| --------------------------------------------------------------------------------------------------------------------- | -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ | -------------- |
| Test that whatever content is displayed in the text area within the text editor is rendered using markdown formatting | Left click a file in the Sidebar | User is navigated to the Text Editor, and the contents of the file are retrieved and _rendered as markdown_ in the Text Editor | **Success** ✅ |
...
| Description                                                                                                                                                                                           | Input                                               | Output                                           | Result                                                                                         |
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------- | ------------------------------------------------ | ---------------------------------------------------------------------------------------------- |
| The central login card should stay a fixed width in the centre of the page, unless the width or height is less than the width or height of the card. In this case, the card should shrink accordingly | Resize the app window when viewing the Login screen | Criteria laid out in the Description must be met | **Partial Success** 🟨 - _components are resizing well, but should have a maximum fixed width_ |
...
| Description                                                                                                                                                                                            | Input                                                | Output                                           | Result                                                                                         |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------- | ------------------------------------------------ | ---------------------------------------------------------------------------------------------- |
| The central signup card should stay a fixed width in the centre of the page, unless the width or height is less than the width or height of the card. In this case, the card should shrink accordingly | Resize the app window when viewing the Signup screen | Criteria laid out in the Description must be met | **Partial Success** 🟨 - _components are resizing well, but should have a maximum fixed width_ |
...
| Description                                                                                                                                                                                                            | Input                                                                                   | Output                                           | Result                                                                                                                                                                                                                                         |
| ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------- | ------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| The sidebar should stay a on the left side of the screen with a margin from the top, bottom, and left of the screen. If the height of the screen shrinks, it should do the same in order to maintain the margin indent | Resize the app window when viewing the Sidebar (part of the Home screen or Text Editor) | Criteria laid out in the Description must be met | **Partial Success** 🟨 - _the Sidebar's height is independent of the height of the screen, and shrinking just causes the content to go off the bottom of the page. However, it retains margins to the left and top, and its width stays fixed_ |
...
I didn't currently have a good enough understanding of CSS to know how to fix the issue. Therefore, I decided it would be better to go away and learn CSS over time, and come back to it when I better understood the language I was working with - this turned out to be in Iteration 2.

##### Test 14d: Home screen resizing

| Description                                                                                                                                                                                             | Input                                              | Output                                           | Result         |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------- | ------------------------------------------------ | -------------- |
| The content of the Home screen should fit within the remaining area of the screen (the area not taken up by the Sidebar), and text should remain a constant width regardless of the width of the screen | Resize the app window when viewing the Home screen | Criteria laid out in the Description must be met | **Success** ✅ |
...
| Description                                                                                                                                           | Input                                                     | Output                                           | Result         |
| ----------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------- | ------------------------------------------------ | -------------- |
| The content of the text editor should shrink to a minimum width, and the text should overflow off the screen if its height exceeds that of the screen | Resize the app window when viewing the Text Editor screen | Criteria laid out in the Description must be met | **Success** ✅ |
...
I plan on addressing theming most likely in Iteration 3, where I have a more complete app with which I can tweak colours and styles more easily, as well as potentially adding different themes.
